...
通用部分已经整理成公开项目[“AI 视频制作助手”](../README.md)，GitHub 仓库名为 `ai-video-maker`，可安装 Skill 的调用名为 `make-ai-video`。仓库根目录保留公开说明、测试和本文；Skill 保持标准目录结构。
...
再把 `make-ai-video/` 安装到 Skill 目录，复制示例生产包，并把输入换成自己的想法、文章、大纲、脚本、资料包或音频。它的原则是“从一个想法，到一条由你确认的视频”。校验通过只表示合同成立；它不会自动把状态推进到人工审核，更不会把本地文件说成已经发布。
